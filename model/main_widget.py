...
class MainWindow(QWidget):
    ...
    def loginFunction(self):
        user_mes = {
            ...
            'PASSWORD': database_try .encrypt(self.login.passwordInput.text())
        }
        # self.user = database.signin(user_mes)
        self.user = database_try .signin(user_mes)
    ...
    def signupFunction(self):
        ...
        res = database_try .check_user_info(self.user)
        if res['res'] == 'fail':
            self.errorBox(res['reason'])
            return
        self.user['MAX'] =10 # 最大规定为10本
        # self.user['PASSWORD'] = database.encrypt(self.user['PASSWORD'])
        self.user['PASSWORD'] = database_try .encrypt(self.user['PASSWORD'])

        # ans = database.signup(self.user)
        ans = database_try.signup(self.user)
        self.user['class'] = 'stu'
