...
class Books(QGroupBox):
    ...
    def borrowBook(self, BID: str):
        ...
        ans = database_try .borrow_book(BID, self.stu_mes['SID'])
        # 刷新表格
        if ans:
            self.searchFunction()
...
class BorrowingBooks(QGroupBox):
    ...
    def setTable(self, val: dict = None):
        ...
        self.book_list = database_try .get_borrowing_books(self.stu_mes['SID'])
        for i in self.book_list:
            self.insertRow(i)
    ...
    def insertRow(self, val: list):
        ...
        isPunished = database_try .days_between(
    ...
    def retrurnBook(self, BID: str):
       #  ans = database.return_book(BID, self.stu_mes['SID'])
        ans = database_try .return_book(BID, self.stu_mes['SID'])
        # 刷新表格
        if ans:
            # self.book_list = database.get_borrowing_books(self.stu_mes['SID'])
            self.book_list = database_try .get_borrowing_books(self.stu_mes['SID'])
    ...
    def pay(self, BID: str, PUNISH):
        ...
        ans = database_try .pay(BID, self.stu_mes['SID'], PUNISH)
        # 刷新表格
        if ans:
            # self.book_list = database.get_borrowing_books(self.stu_mes['SID'])
            self.book_list = database_try .get_borrowing_books(self.stu_mes['SID'])
...
class History(QGroupBox):
    ...
    def setTable(self, val: dict = None):
        ...
        self.list = database_try .get_log(self.stu_mes['SID'])
        for i in self.list:
            self.insertRow(i)
        self.body.addWidget(self.table)
...
class Detial(QWidget):
    def __init__(self, stu_mes):
        ...
        self.stu_mes = database_try .get_student_info(stu_mes['SID'])
    ...
    def saveFunction(self):
        ...
        if self.passwordInput.text() != '******':
            ...
            self.stu_mes['PASSWORD'] = database_try .encrypt(self.passwordInput.text())
        ...
        if not database_try .update_student(self.stu_mes):
            print('更新失败')
            return
        self.save.setEnabled(False)
        self.nameInput.setEnabled(False)
